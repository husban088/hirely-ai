...
export class JobsService {
  constructor(
    @InjectModel(JobApplication.name) private jobModel: Model<JobApplicationDocument>,
  ) {}
  ...
  async findAllForUser(userId: string) {
    return this.jobModel.find({ userId: new Types.ObjectId(userId) }).sort({ updatedAt: -1 });
  }

  async update(userId: string, input: UpdateJobInput) {
    const job = await this.findOwned(userId, input.id);
    Object.assign(job, input);
    if (input.status === 'APPLIED' && !job.appliedDate) job.appliedDate = new Date();
    return job.save();
  }
  ...
  private async findOwned(userId: string, id: string) {
    ...
    if (job.userId.toString() !== userId) throw new ForbiddenException('Not your job application.');
    return job;
  }
}
