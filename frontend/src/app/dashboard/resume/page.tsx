...
export default function ResumePage() {
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      const isLocalBackend = /localhost|127\.0\.0\.1/.test(UPLOAD_URL);
      if (!isLocalBackend && file.size > 4 * 1024 * 1024) {
        toast.error("File is too large. Please upload a PDF/DOCX under 4 MB.");
        return;
      }
      ...
      try {
        ...
        if (!res.ok) {
          ...
          try {
            ...
            if (res.status === 413) message = "File is too large to upload.";
          }
